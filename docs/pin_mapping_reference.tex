...
(\code{SOURce:ENAble?}) -- confirmed on real hardware: with only
...
\begin{longtable}{p{1.0in} p{0.5in} p{0.55in} p{2.6in}}
  ...
  \code{GPInput\_12} & PF13 & GPI & External-enable interlock. \code{ARM}/
    \code{SHOT:STARt} refuse unless HIGH; continuously monitored
  ...
  \code{Fiber\_Enable} & PF15 & GPI & External trigger. A rising edge
    ...
    \code{SHOT:STARt} uses). Independent of the interlock above
