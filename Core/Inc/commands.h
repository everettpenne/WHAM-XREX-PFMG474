 ...
 *   11  Invalid channel -- must be 1-HRTIM_NUM_CHANNELS (see
 *       CONFig:CHANnels?, pid.h)
 *   12  Invalid command arguments (wrong count, or a non-numeric/
 *       out-of-range value)
 *   13  Invalid state-machine transition for the current state -- ARM
 *       (not currently IDLE, or a channel not ready), DISARM, or a
 *       fire attempt (FIRE/SOURce:RUN/SHOT:STARt) sent while not
 *       ARMED -- see state_machine.h. Widened 2026-09-22 when the
 *       ARM-state precondition was added to FIRE/SOURce:RUN alongside
 *       SHOT:STARt -- all three (plus ARM itself) can generate this.
 *   14  Invalid CHANnel:NICKname -- 1-PID_CHANNEL_NICKNAME_MAX_LEN
 *       chars, no whitespace, and not the reserved value "-" (pid.h)
 *   15  A fire attempt (FIRE/SOURce:RUN/SHOT:STARt) or ARM itself
 *       refused -- the external-enable interlock (EXTernal:ENAble) is
 *       on and PF13 currently reads LOW (state_machine.h). Widened
 *       2026-09-22 alongside code 13 above -- previously SHOT:STARt-only.
 ...
 * Legacy-path-only (TABle:STEP's own ceiling). See commands.c's own
 * header comment on PFM_SetMaxCarrierFreqHz(). */
void cmd_config_max_carrier_hz(uart_instance_t *inst, char *args);       /* CONFig:
                                                                             MaxCarrierHz <hz>
                                                                             -- OK, or ERR 12 */
void cmd_config_max_carrier_hz_query(uart_instance_t *inst, char *args); /* CONFig:
                                                                             MaxCarrierHz? --
                                                                             OK <hz> */
...
void cmd_disarm(uart_instance_t *inst, char *args);       /* DISARM -- OK, ARMED -> IDLE,
                                                                no-op if not ARMED */
void cmd_debug_fault_bypass(uart_instance_t *inst, char *args);       /* DEBUG:FAULT:BYPASS
                                                                            <0|1> -- OK; bench-
                                                                            only, see
                                                                            state_machine.h */
void cmd_debug_fault_bypass_query(uart_instance_t *inst, char *args); /* DEBUG:FAULT:BYPASS?
                                                                            -- OK <0|1> */
...
/* Telemetry contract (docs/telemetry.md, Phase 1), `SYS:` namespace. */
void cmd_sys_time(uart_instance_t *inst, char *args);       /* SYS:TIME? -- OK <ms-since-boot> */
void cmd_sys_telem(uart_instance_t *inst, char *args);      /* SYS:TELEM? -- OK <schema_version> */
void cmd_sys_event(uart_instance_t *inst, char *args);      /* SYS:EVENT <0|1> -- OK (gate !EVT) */
void cmd_sys_event_query(uart_instance_t *inst, char *args);/* SYS:EVENT? -- OK <0|1> */
void cmd_sys_evlog(uart_instance_t *inst, char *args);      /* SYS:EVLOG? -- OK <n> + n !EVT lines
                                                                    (flight recorder, Phase 4) */
 ...
 * reasoning as ARM/DISARM above. ERR 15 (new): SHOT:STARt
 ...
 * usable GPIO -- see state_machine.h / docs/changelog.txt). A future
 * E-stop must use a genuinely free input pin. */
 ...
 * PID_SetLoopRateHz()'s own doc comment (pid.h/pid.c). */
void cmd_config_pid_rate(uart_instance_t *inst, char *args);       /* CONFig:PIDRate <hz>
                                                                        -- OK, or ERR 12 if
                                                                        rejected (range,
                                                                        hardware check, or
                                                                        a shot is running) */
void cmd_config_pid_rate_query(uart_instance_t *inst, char *args); /* CONFig:PIDRate? --
                                                                        OK <hz> */

/* CONFig:TURNONHz / CONFig:MAXFREQHz <hz> / ? -- added 2026-09-22,
 * direct request: PFM_TURNON_FREQ_HZ/PFM_MAX_FREQ_HZ (ctrlr_config.h)
 * made runtime-configurable. See PID_SetTurnonFreqHz()/
 * PID_SetMaxFreqHz()'s own doc comments (pid.h/pid.c) for the
 * cross-validation each applies. */
void cmd_config_turnon_hz(uart_instance_t *inst, char *args);        /* CONFig:TURNONHz <hz>
                                                                          -- OK, or ERR 12 */
void cmd_config_turnon_hz_query(uart_instance_t *inst, char *args);  /* CONFig:TURNONHz? --
                                                                          OK <hz> */
void cmd_config_max_freq_hz(uart_instance_t *inst, char *args);      /* CONFig:MAXFREQHz <hz>
                                                                          -- OK, or ERR 12 */
void cmd_config_max_freq_hz_query(uart_instance_t *inst, char *args); /* CONFig:MAXFREQHz? --
                                                                           OK <hz> */

/* CONFig:MAXCURRent <ch> <amps> / <ch> -- added 2026-09-22, direct
 * request: PFM_MAX_CURRENT_A_PER_CHANNEL[ch] (ctrlr_config.h) made
 * runtime-configurable. See PID_SetMaxCurrentA()'s own doc comment
 * (pid.h/pid.c). */
void cmd_config_max_current(uart_instance_t *inst, char *args);       /* CONFig:MAXCURRent
                                                                           <ch> <amps> -- OK,
                                                                           or ERR 11/12 */
void cmd_config_max_current_query(uart_instance_t *inst, char *args); /* CONFig:MAXCURRent?
                                                                           <ch> -- OK <amps> */
 ...
 * (pid.h/pid.c). */
void cmd_config_slew_rate(uart_instance_t *inst, char *args);       /* CONFig:SLEWRate
                                                                         <hzPerTick> -- OK,
                                                                         or ERR 12 */
void cmd_config_slew_rate_query(uart_instance_t *inst, char *args); /* CONFig:SLEWRate? --
                                                                         OK <hzPerTick> */

/* CONFig:FaultRampTime <s> / ? -- added 2026-09-22, direct request:
 * FAULT_RAMP_DOWN_TIME_S (ctrlr_config.h) made runtime-configurable.
 * See PID_SetFaultRampDownTimeS()'s own doc comment (pid.h/pid.c). */
void cmd_config_fault_ramp_time(uart_instance_t *inst, char *args);       /* CONFig:
                                                                              FaultRampTime <s>
                                                                              -- OK, or ERR 12 */
void cmd_config_fault_ramp_time_query(uart_instance_t *inst, char *args); /* CONFig:
                                                                              FaultRampTime? --
                                                                              OK <s> */

/* CONFig:FaultPolarity:WATER/:TEMP/:ENERPRO/:OCP <0|1> / ? -- added
 * 2026-09-22, direct request: the four XR_*_FLT_POLARITY constants
 * (ctrlr_config.h) made runtime-configurable. <0|1> = FAULT_POLARITY_
 * NORMALLY_HIGH/_LOW's own encoding. See commands.c's own header
 * comment on ConfigFaultPolaritySet(). */
 ...
 * signal for a given Transrex. Raw HIGH/LOW levels only, same "raw
 * readback" convention GDS?/EXTernal:INPut? already use --
 * ctrlr_config.h's XR_WATER_FLT_POLARITY/etc. are what decide which
 * level actually means "faulted," not this command.
 ...
 * namespaces, added 2026-09-09 (PID control) and extended through
 * 2026-09-22. See pid.h for the full architecture (Possibility 3 +
 * fixed-rate Master heartbeat) this project exists to implement, and
 * docs/changelog.txt's design-decision entry for the reasoning.
 * Channel numbering matches PFMIN:DATA?'s own convention: 1..N on the
 * wire (N = HRTIM_NUM_CHANNELS, CONFig:CHANnels? reports it), 0..N-1
 * internally. Not gated on a feature-enable flag -- this project's
 * whole point, unlike PFM_Input/QUADSPI/BOOT's opt-in modules.
 *
 * RENAMED 2026-09-22: the output/demand commands moved out of PID: into
 * SOURce:/SHOT:/LOG:/CHANnel:, leaving PID: to mean only the actual
 * PID-loop parameters (PID:GAINS/PID:LOOPMODE below). Clean
 * cut -- no aliases; the old mnemonics are gone. The underlying
 * pid.c API (PID_Start(), PID_SetSetpoint(), etc.) is unchanged. */
void cmd_source_run(uart_instance_t *inst, char *args);     /* SOURce:RUN -- OK, begins
                                                                 closed-loop operation on
                                                                 every channel */
void cmd_source_stop(uart_instance_t *inst, char *args);    /* SOURce:STOP -- OK, stops
                                                                 output + feedback capture */
void cmd_source_setpoint(uart_instance_t *inst, char *args);/* SOURce:SETpoint <ch> <hz> --
...
void cmd_pid_gains_query(uart_instance_t *inst, char *args); /* PID:GAINS? <ch> -- OK
                                                                  ...
                                                                  2026-09-10, see
                                                                  PID_GetGains() */
void cmd_source_status(uart_instance_t *inst, char *args);  /* SOURce:STATus? <ch> -- OK
                                                                 <running> <setpointHz>
                                                                 <measuredHz> <outputHz> */

/* Waveform logging, added 2026-09-10 -- see pid.h's own comment block
   on PID_ArmLog() for the full design (why polling SOURce:STATus? isn't
   ...
   comparison. */
void cmd_log_arm(uart_instance_t *inst, char *args);        /* LOG:ARM <ch(0=all)>
                                                                 <maxSamples> <decim> -- OK,
                                                                 arms waveform logging */
void cmd_log_data(uart_instance_t *inst, char *args);       /* LOG:DATA? [ch] -- OK <count>
...
void cmd_source_ramp(uart_instance_t *inst, char *args);    /* SOURce:RAMP <ch> <startHz>
                                                                 <endHz> <durationMs> --
                                                                 OK, begins a linear
                                                                 setpoint ramp */
...
/* Per-channel output enable/disable, added 2026-09-11 per direct
   ...
   PFM waveform"). */
void cmd_source_enable(uart_instance_t *inst, char *args);       /* SOURce:ENAble
                                                                      <ch> <0|1> -- OK,
                                                                      0 = output fully
                                                                      disabled (no PFM
                                                                      waveform at all),
                                                                      1 = enabled
                                                                      (default) */
void cmd_source_enable_query(uart_instance_t *inst, char *args); /* SOURce:ENAble?
                                                                      <ch> -- OK <0|1> */
...
void cmd_chan_nickname(uart_instance_t *inst, char *args);       /* CHANnel:NICKname
                                                                      <ch> <name> -- OK,
                                                                      or ERR 14 if name
                                                                      is invalid */
void cmd_chan_nickname_query(uart_instance_t *inst, char *args); /* CHANnel:NICKname?
                                                                      <ch> -- OK <name>,
                                                                      or OK - if none set */

void cmd_shot_timing(uart_instance_t *inst, char *args);  /* SHOT:TIMing
                                                               <rampUpTimeS>
                                                               <flatTopTimeS>
                                                               <rampDownTimeS> --
                                                               OK, sets the SHARED
                                                               ...
                                                               shared ramp, before
                                                               this) */
void cmd_shot_timing_query(uart_instance_t *inst, char *args); /* SHOT:TIMing?
                                                                    -- OK <rampUpTimeS>
                                                                    <flatTopTimeS>
                                                                    <rampDownTimeS> --
                                                                    added 2026-09-10,
                                                                    ...
                                                                    ERR 12 if never set */
void cmd_shot_current(uart_instance_t *inst, char *args); /* SHOT:CURRent
                                                               <ch> <demandCurrentA>
                                                               -- OK, sets channel
                                                               ch's peak current
                                                               for the next shot */
void cmd_shot_current_query(uart_instance_t *inst, char *args); /* SHOT:CURRent?
                                                                     <ch> -- OK
                                                                     <demandCurrentA>
                                                                     -- added 2026-09-10,
                                                                     see
                                                                     PID_GetProfileCurrent() */
void cmd_shot_start(uart_instance_t *inst, char *args);   /* SHOT:STARt --
                                                               OK, begins a
                                                               profiled shot on
                                                               every channel at
                                                               once */
...
void cmd_sim_model_tau_query(uart_instance_t *inst, char *args);       /* SIM:MODEL:TAU? --
                                                                            OK <ms> */
void cmd_sim_diag_idletone(uart_instance_t *inst, char *args);         /* SIM:DIAGnostic:IDLETONE
                                                                            <0|1> -- OK; board-wide,
                                                                            see sim_transrex.h */
void cmd_sim_diag_idletone_query(uart_instance_t *inst, char *args);   /* SIM:DIAGnostic:IDLETONE?
                                                                            -- OK <0|1> */
